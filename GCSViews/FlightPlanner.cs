...
namespace MissionPlanner.GCSViews
{
    public partial class FlightPlanner : MyUserControl, IDeactivate, IActivate
    {
        ...
        //Maximum three wehicles are used
        private int[] vehicleIdList = { 0, 0, 0 };
        private int primaryIndex = -1 ;
        private int secondaryIndex = -1;
        private int tertiaryIndex = -1;

        public int[] routeTimes = { 0, 0, 0 };
        ...
        public void callMeDrag(string pointno, double lat, double lng, int alt)
        {
            ...
            if (selectedrow != 0)
            {
                if (Commands.Rows[selectedrow - 1].Cells[Command.Index].Value.ToString() == MAVLink.MAV_CMD.DO_SEND_SCRIPT_MESSAGE.ToString())
                {
                    ...
                    Commands.Rows[selectedrow - 1].Cells[Param2.Index].Value = (distance / speed).ToString("F0");
        ...
        public void writeKML()
        {
            ...
            try
            {
                ...
                if ((MAVLink.MAV_MISSION_TYPE) cmb_missiontype.SelectedValue == MAVLink.MAV_MISSION_TYPE.MISSION)
                {
                    ...
                    //Check for indexes if they are -1 or not
                    WPOverlay wpOverlay2 = new WPOverlay();
                    wpOverlay2.overlay.Id = "WPOverlaySecondary";

                    if (secondaryIndex != -1)
                    {
                        wpOverlay2.CreateOverlay(home,
                                wplists[secondaryIndex],
                                double.Parse(TXT_WPRad.Text) / CurrentState.multiplieralt,
                                double.Parse(TXT_loiterrad.Text) / CurrentState.multiplieralt, CurrentState.multiplieralt, .5f,false);
                    }

                    WPOverlay wpOverlay3 = new WPOverlay();
                    wpOverlay3.overlay.Id = "WPOverlayTertiary";

                    if (tertiaryIndex != -1)
                    {
                        wpOverlay3.CreateOverlay(home,
                                wplists[tertiaryIndex],
                                double.Parse(TXT_WPRad.Text) / CurrentState.multiplieralt,
                                double.Parse(TXT_loiterrad.Text) / CurrentState.multiplieralt, CurrentState.multiplieralt, .5f,false);
                    }
        ...
        private void processToScreen(List<Locationwp> cmds, bool append = false, bool removehome = true)
        {
            ...
            if (!append && type == MAVLink.MAV_MISSION_TYPE.MISSION && removehome)
            {
                try
                {
...
Column 1: Field type (RALLY is the only one at the moment -- may have RALLY_LAND in the future)
 ...
 Column 7: Flags (just 0 for now, also future use).
        ...
        {
            //Console.WriteLine("UpdateVehicleMissionOnScreen called with id {0}", sysid);

            var newIndex = getSysIdIndex(sysid);
            if (newIndex == -1)
            {
                CustomMessageBox.Show("Invalid SysID passed to FlightPlanner (ID " + sysid.ToString() + " )");
                return;
            }
            //Missions stored in a wplists array where the array index matches the indes on the vehicleIdList index of the given sysid
            //First we have to save the current mission to the appropiate list, to store changes in the list that are not yet sent to the vehicle
            var currentIndex = getSysIdIndex(currentSysId);

            //Copy current list ti the currentIndex 
            wplists[currentIndex] = GetCommandList();

            //Update list with the new vehicle mission;
            processToScreen(wplists[newIndex], false, false);  // Do we need to remove home ???


            //Setup secondary and tertiary displays
            if (secondaryIndex == newIndex)
            {
                secondaryIndex = currentIndex;
            }
            if (tertiaryIndex == newIndex)
            {
                tertiaryIndex = currentIndex;
            }

            //finally set currentSysID
            currentSysId = sysid;
            writeKML();
            MainV2.instance.FlightPlanner.MainMap.Position = MainV2.instance.FlightPlanner.MainMap.Position;

            //Console.WriteLine("new {0} secondary {1} tertiary {2}", newIndex, secondaryIndex, tertiaryIndex);

        }


        public void UpdateVehicleIdList(int sysid1, int sysid2, int sysid3)
        {
            vehicleIdList[0] = sysid1;
            vehicleIdList[1] = sysid2;
            vehicleIdList[2] = sysid3;
            //Console.WriteLine("UpdateVehicleIDList called with ids {0} {1} {2}", sysid1,sysid2, sysid3);

            currentSysId = sysid1;
            secondaryIndex = 1;
            tertiaryIndex = 2;

            lOffset1.Text = $"ID{vehicleIdList[1]}";
            lOffset2.Text = $"ID{vehicleIdList[2]}";
        ...
        public void updatetime()
        {
            ...
            int currentIndex = getSysIdIndex(currentSysId);
            routeTimes[currentIndex] = 0;
       ...
       public void updatealltime()
        ...
        private void bGenerateFormation_Click(object sender, EventArgs e)
        {
            ...
            {
                CustomMessageBox.Show("You have to select the primary (first on top) vehicle as a lead vehicle");
                return;
            }

            if (vehicleIdList[1] <= 0)
            {
                CustomMessageBox.Show("You need at least two vehicles connected to create a close formation");
                return;
            }
